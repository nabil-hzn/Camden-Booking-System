...
export default function RoomCard({ room, isSelected, onSelect, selectedDate, bookings }: RoomCardProps) {
  ...
  return (
    <button
      onClick={onSelect}
      className={`
        w-full text-left relative overflow-hidden rounded-xl border transition-all duration-300 flex flex-col px-2.5 py-1.5 sm:px-3 sm:py-2 md:py-2.5 cursor-pointer group focus:outline-none justify-between gap-1 sm:gap-1.5 md:gap-2 flex-1 shrink-0 min-h-[108px] sm:min-h-[118px] lg:min-h-[128px]
        ${isSelected 
    ...
    >
      ...
      <div className="relative z-10 w-full text-[8px] sm:text-[9px] md:text-[10px] text-slate-500 font-sans leading-normal min-h-0 flex-1 flex flex-col justify-center">
        {room.type === 'nap' ? (
          <div className="bg-slate-50 border border-slate-100 p-1.5 sm:p-2 rounded-lg w-full">
            ...
            <p className="text-[7px] sm:text-[8px] md:text-[9px] text-slate-400 font-medium italic leading-relaxed">
              Only for doctors and directors of clinics, up to 1 hour per booking/day.
            </p>
          </div>
        ) : (
          <div className="bg-slate-50 border border-slate-100 p-1.5 sm:p-2 rounded-lg space-y-1 sm:space-y-1.5 w-full">
            ...
            <div className="flex justify-between items-center gap-1 text-[7px] sm:text-[8px] md:text-[9px] border-t border-slate-200/50 pt-1 sm:pt-1.5">
