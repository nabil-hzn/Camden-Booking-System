...
export default function RoomCard({ room, isSelected, onSelect, selectedDate, bookings }: RoomCardProps) {
  ...
  return (
    <button
      onClick={onSelect}
      className={`
        w-full text-left relative overflow-hidden rounded-xl border transition-all duration-300 flex flex-col px-2.5 py-1.5 sm:px-3 sm:py-2 md:py-2.5 cursor-pointer group focus:outline-none justify-between gap-1 sm:gap-1.5 md:gap-2 flex-1 h-full min-h-0
        ${isSelected 
    ...
    >
      ...
      {/* Main Info */}
      <div className="relative z-10 w-full min-h-0 flex-shrink-0">
        <h3 className="font-serif italic font-extrabold text-[11px] sm:text-xs md:text-sm lg:text-base text-[#0f172b] leading-tight group-hover:text-[#0a1023] transition-colors">
          {room.name}
        </h3>
      </div>
      ...
      <div className="relative z-10 w-full text-[8px] sm:text-[9px] md:text-[10px] text-slate-500 font-sans leading-normal min-h-0 flex-1 flex flex-col justify-center">
        {room.type === 'nap' ? (
          ...
          <div className="bg-slate-50 border border-slate-100 p-1 sm:p-1.5 rounded-lg space-y-0.5 sm:space-y-1 w-full">
            <div className="flex justify-between items-center text-[7px] sm:text-[8px] md:text-[9px]">
              <span className="text-slate-400 font-medium">Operating hours</span>
              <span className="font-bold text-[#0f172b] font-mono">
                {room.type === 'meeting' ? '$80' : room.type === 'living_meeting' ? '$200' : '$580'}/hr
              </span>
            </div>
            <div className="flex justify-between items-center text-[7px] sm:text-[8px] md:text-[9px] border-t border-slate-200/50 pt-0.5 sm:pt-1">
              <span className="text-slate-400 font-medium">After operating hours</span>
              <span className="font-bold text-[#0f172b] font-mono">
